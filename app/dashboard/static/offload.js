...
let replacingCredentials = false;
...
function render(data) {
  ...
  document.querySelector("#setup-only").hidden = data.service_enabled;
  const credentialStatus = document.querySelector("#credential-status");
  credentialStatus.textContent = data.configured ? "CONFIGURED" : "REQUIRED";
  credentialStatus.className = data.configured ? "status" : "status warning";
  const credentialForm = document.querySelector("#credential-form");
  credentialForm.hidden = data.configured && !replacingCredentials;
  document.querySelector("#credential-details").hidden = !data.configured || replacingCredentials;
  document.querySelector("#credential-file-label").textContent = replacingCredentials ? "Replacement OAuth JSON file" : "OAuth JSON file";
  document.querySelector("#credential-submit").textContent = replacingCredentials ? "Replace OAuth JSON" : "Upload OAuth JSON";
  document.querySelector("#credential-hint").textContent = data.credential?.client_id_hint || "—";
  document.querySelector("#credential-fingerprint").textContent = data.credential?.fingerprint || "—";
  ...
  const cleanup = document.querySelector("#auto-cleanup");
  cleanup.disabled = !data.service_enabled;
  if (!data.service_enabled) cleanup.checked = false;
  document.querySelector("#cleanup-preview-note").hidden = data.service_enabled;
  ...
  document.querySelector("#cleanup-mode").textContent = !data.service_enabled ? "Off during preview setup" : data.auto_cleanup ? "Enabled after full verification" : "Manual";
  ...
  pauseButton.hidden = !data.service_enabled;
  pauseButton.textContent = data.paused ? "Resume uploads" : "Pause uploads";
  pauseButton.dataset.paused = data.paused ? "true" : "false";
  const cleanupButton = document.querySelector("#toggle-cleanup");
  cleanupButton.hidden = !data.service_enabled;
  cleanupButton.textContent = data.auto_cleanup ? "Disable automatic cleanup" : "Enable verified cleanup";
  cleanupButton.dataset.enabled = data.auto_cleanup ? "true" : "false";
...
document.querySelector("#credential-form").addEventListener("submit", async event => {
  ...
  if (oauthConfigured && replacingCredentials && !confirm("Replace the OAuth application? The current Google account will be disconnected.")) return;
  const form = new FormData();
  const file = document.querySelector("#credential-file").files[0];
  if (!file) return showToast("Choose the OAuth JSON downloaded from Google Cloud");
  form.append("credentials", file);
  if (oauthConfigured) form.append("confirm", "REPLACE");
  const response = await fetch("/api/offload/credentials", {method: "POST", body: form});
  ...
  showToast("Google OAuth application configured on this Pi");
  await refresh();
});

document.querySelector("#replace-credentials").addEventListener("click", () => {
  replacingCredentials = true;
  renderFormState();
});

function renderFormState() {
  document.querySelector("#credential-form").hidden = false;
  document.querySelector("#credential-details").hidden = true;
  document.querySelector("#credential-file-label").textContent = "Replacement OAuth JSON file";
  document.querySelector("#credential-submit").textContent = "Replace OAuth JSON";
  document.querySelector("#credential-file").focus();
}

document.querySelector("#remove-credentials").addEventListener("click", async () => {
  if (!confirm("Remove the OAuth application and disconnect Google Drive from this Pi?")) return;
  const response = await fetch("/api/offload/credentials/remove", {
    method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({confirm: "REMOVE"}),
  });
  const result = await response.json();
  if (!response.ok || !result.ok) return showToast(result.error || `Status ${response.status}`);
  replacingCredentials = false;
  showToast("OAuth application removed from this Pi");
  await refresh();
});
...
document.querySelector("#toggle-cleanup").addEventListener("click", async event => {
  const enabled = event.currentTarget.dataset.enabled !== "true";
  if (enabled && !confirm("Enable automatic local video removal after an entire date is verified in Google Drive?")) return;
  const response = await fetch("/api/offload/cleanup", {
    method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify({enabled}),
  });
  const result = await response.json();
  if (!response.ok || !result.ok) return showToast(result.error || `Status ${response.status}`);
  showToast(enabled ? "Verified cleanup enabled" : "Automatic cleanup disabled");
  await refresh();
});
