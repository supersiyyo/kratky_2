...
def test_recordings_page_lists_only_completed_combined_timelapses(
    tmp_path: Path,
) -> None:
    config = config_from_mapping(valid_mapping(tmp_path))
    ...
    assert b"Completed timelapses" in index_response.data
    assert b"2026-07-25" in index_response.data
    assert b"Download timelapse" in index_response.data
    assert b"Download complete day" not in index_response.data
    assert b"water-2026-07-25_08-00-00.mkv" not in index_response.data
    assert detail_response.status_code == 200
    assert b"Download timelapse" in detail_response.data
    assert b"Open / download" not in detail_response.data
...
def test_completed_combined_timelapse_is_previewed_on_its_recording_day(
    ...
    assert b"Completed timelapse" in detail.data
    assert b"<video controls" in detail.data
    assert b"Download timelapse" in detail.data
    assert video.status_code == 200
    assert video.mimetype == "video/mp4"
    assert video.data == b"combined-video"


def test_missing_combined_timelapse_is_not_listed_or_opened(tmp_path: Path) -> None:
    config = config_from_mapping(valid_mapping(tmp_path))
    first = datetime(2026, 7, 25, 8, tzinfo=ZoneInfo("America/Los_Angeles"))
    _complete_recording_day(config, first)
    client = create_app(config).test_client()
    ...
    index = client.get("/recordings")
    assert index.status_code == 200
    assert b"2026-07-25" not in index.data
    assert detail.status_code == 404
    assert video.status_code == 404


def test_completed_timelapse_remains_available_after_raw_cleanup(
    tmp_path: Path,
) -> None:
    config = config_from_mapping(valid_mapping(tmp_path))
    combined = combined_timelapse_path(config, "2026-07-25")
    ...
    index = client.get("/recordings")
    detail = client.get("/recordings/2026-07-25")
    raw_file = client.get("/recordings/file/2026-07-25/water/hour.mkv")
    raw_review = client.get("/recordings/review/2026-07-25/water/hour.mkv")
    archive = client.get("/recordings/archive/2026-07-25.zip")

    assert index.status_code == 200
    assert b"2026-07-25" in index.data
    assert detail.status_code == 200
    assert raw_file.status_code == 404
    assert raw_review.status_code == 404
